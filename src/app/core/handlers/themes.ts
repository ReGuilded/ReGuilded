 ...
 */
export default class ThemeHandler extends ExtensionHandler<Theme> {
                ...
                new Promise((resolve, reject) => {
                    ...
                    for (let file of propFiles) {
                        ...
                        if (!existsSync(filePath))
                            return reject(
                                new Error(`Could not find CSS file in path ${filePath}`)
                            );
                    }
        ...
        readFile(
            ...
            (err, file) => {
                ...
                if (typeof json !== "object")
                    return console.warn(
                        "Expected theme settings to be of type 'object' in theme",
                        metadata.id
                    );
    ...
    assignProperties(metadata: Theme, props: { [prop: string]: string | number | boolean }) {
        for (let key of Object.keys(props)) metadata.settings[key].value = props[key];
