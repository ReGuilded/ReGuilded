...
export default class GeneralSettings extends React.Component {
    ...
    private *onSaveChanges({ changedValues, values, isValid }) {
        if (isValid) {
            const {
                loadAuthors,
                loadImages,
                badge: { optionName: badge },
                keepSplash,
                autoUpdate
            }: GeneralSettingsValues = values;
            ...
            yield window.ReGuilded.settingsHandler.update(configValues).then(() => {
    ...
    private async onUpdate() {
        ...
        await window.ReGuildedConfig.checkForUpdate().then(async ([updateExists, updateInfo]) => {
                    ...
                    async ({ confirmed }) =>
                        ...
                        (await window.ReGuildedConfig.doUpdateIfPossible()
                            .then((isUpdate) => {
                                switch (isUpdate) {
                                    case true:
                                        statusContext.displayStatus({
                                            text: "ReGuilded update finished. CTRL + R to refresh",
                                            type: "success"
                                        });
                                        break;
                                    case false:
                                        statusContext.displayError({
            ...
            // Notify that there is no update
                ...
                statusContext.displayStatus({
                    text: "No ReGuilded update has been found",
                    type: "info"
                });
        });
    }
            ...
            <ErrorBoundary>
                <div className="ReGuildedSettings-container">
                    ...
                    <GuildedText block type="heading3" className="SettingsHeaderWithButton-header ReGuildedSettings-header">
                        ReGuilded General Settings
                    </GuildedText>
                    <div className="ReGuildedSettings-section ReGuildedSettings-info">
                        {/* TODO: Injection of ReGuilded icon */}
                        <IconAndLabel
                            className="ReGuildedSettings-info-item"
                            iconName="icon-star"
                            label={["ReGuilded version: ", <InlineCode>{window.ReGuilded.version}</InlineCode>]}
                        />
                        <IconAndLabel
                            className="ReGuildedSettings-info-item"
                            iconName="brand-logomark"
                            label={["Guilded version: ", <InlineCode>{window.GuildedNative?.appVersion}</InlineCode>]}
                        />
                        <IconAndLabel
                            className="ReGuildedSettings-info-item"
                            iconName="icon-desktop"
                            label={["Electron version: ", <InlineCode>{window.GuildedNative?.electronVersion}</InlineCode>]}
                        />
                    </div>
                                        ...
                                        fieldSpecs: [
                                            {
                                                ...
                                                label: "Auto-Update",
                                                description:
                                                    "Every time Guilded is launched or gets refreshed, ReGuilded checks for its own updates and installs them if they exists.",
