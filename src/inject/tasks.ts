...
export function inject(
    ...
    return new Promise<void>((resolve, reject) => {
        ...
        copy(src, join(platform.reguildedDir, "reguilded.asar"), { recursive: true, errorOnExist: false, overwrite: true }, (err) => {
            if (err) reject(err);
...
export async function prepareAndPackResources() {
    return new Promise<void>((resolve, reject) => {
        writeFile(join(__dirname, "app", "package.json"), `{"name":"reguilded","main":"electron.patcher.js", "version":"${reGuildedInfo.version}"}`, "utf8").then(() =>
