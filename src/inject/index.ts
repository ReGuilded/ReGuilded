    ...
    new Promise<boolean>((resolve, reject) => {
        if (specialTasks.includes(taskArg)) {
            access(platform.appDir, constants.F_OK, err => {
                ...
                } else if (["linux", "darwin"].includes(process.platform) && process.getuid() && process.getuid() != 0)
                    resolve(true);
                else resolve(false);
            });
        } else resolve(false);
        ...
        .then(requireElevate => {
            if (requireElevate) {
                console.error(
                    `Task ${taskArg}, requires elevated privileges, please complete the prompt that has opened.`
                );
            ...
            if (tasks[taskArg] != null) {
                ...
                new Promise<void>(taskResolve => {
                    ...
                    tasks[taskArg](platform, "sudo")
                        .then(() => {
                            console.info(
                                `Task ${taskArg} is complete${
                                    !restartNeeded ? "." : ", and you can now relaunch Guilded."
                                }`
                            );
